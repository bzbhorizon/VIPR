...
public class Hub implements Runnable {
		...
		try {
			...
			new Thread(new Runnable() {
				public void run() {
					...
					try {
						System.out
								.println("Started main Google Earth installation display on "
	...
	public void run() {
		new Thread(new Runnable() {
			public void run() {
				Iterator<ArrayList<String>> i = Meta.getSubScreens()
						.iterator();
				while (i.hasNext()) {
					String ip = "";
					int port = -1;
					try {
						...
						System.out
								.println("Connected to secondary Google Earth installation display on "
										+ ip + ":" + port);
					} catch (IOException e) {
						e.printStackTrace();
						System.out
								.println("Couldn't connect to secondary display on "
										+ ip + ":" + port);
					}
				}
			}
	...
	private class ProcessThread implements Runnable {
		...
		private void informSubs(String message) {
			for (int i = 0; i < dos.size(); i++) {
				try {
	...
	public void updatePosition() {
		try {
			...
			if (!(dx < 0.15 && dx > -0.15) || !(dy < 0.15 && dy > -0.15)) {
				dx = delay * SPEED_BASE * Meta.getAltitudeForLevel(alt)
						/ Meta.getAltitudeForLevel(0) * dx;
